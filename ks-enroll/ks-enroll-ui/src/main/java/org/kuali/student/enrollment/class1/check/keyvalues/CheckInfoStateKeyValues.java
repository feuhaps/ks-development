 ...
 */
public class CheckInfoStateKeyValues extends UifKeyValuesFinderBase implements Serializable {
    ...
    public List<KeyValue> getKeyValues(ViewModel model) {
        ...
        try {
            List<StateInfo> states = getStateService().getStatesByLifecycle(ProcessServiceConstants.CHECK_LIFECYCLE_KEY, context);
            for (StateInfo state : states) {
                if(state.getKey().startsWith("kuali.process.check.state")) { //TODO remove check after data is fixed
