...
/**
 * Duplicate override to fix wildcard searching not ignoring case. (used to work in OJB days) See line 136
 * See
 * https://jira.kuali.org/browse/KSENROLL-12762
 * https://jira.kuali.org/browse/KULRICE-12642
 *
 */
    ...
    private static Predicate parsePredicate(String key, String value) {
        if (value.contains(SearchOperator.NULL.op())) {
            ...
            if (isNot(value)) {
                ...
                return likeIgnoreCase(key, value);//THIS IS THE LINE THAT CHANGED
            }
        } else {
            if (isNot(value)) {
